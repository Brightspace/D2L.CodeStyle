...
void Bar() {
  ...
  [Test]
	...
	public void TryFinally() {
		var actual = Transform( @"[GenerateSync] async Task BarAsync() { try { response = await GetBazAsync().ConfigureAwait( false ); } finally { await FinalizeBazAsync(); } }" );

		Assert.IsTrue( actual.Success );
		Assert.IsEmpty( actual.Diagnostics );
		Assert.AreEqual( @"[Blocking] void Bar() { try { response = GetBaz(); } finally { FinalizeBaz(); } }", actual.Value.ToFullString() );
	}
